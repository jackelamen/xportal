...
const NAV = [
  { href: "/portal", icon: "LayoutDashboard", key: "nav.home" },
...
export default async function PortalLayout({ children }) {
  ...
  const nav = NAV.map((n) => ({ ...n, label: t(locale, n.key) }));
    ...
    <div style={client.accent_color ? { "--accent": client.accent_color } : undefined}>
      {session.preview && (
        <div className="sticky top-0 z-40 flex items-center justify-center gap-3 bg-warn px-4 py-2 text-sm font-medium text-white">
          {t(locale, "layout.previewBanner", { company: client.company_name, name: user.name })}
      ...
      <div className="flex min-h-screen flex-col md:flex-row">
        ...
        <header className="flex items-center justify-between bg-[#0b0d13] p-4 text-white md:hidden">
          <BrandMark client={client} locale={locale} compact />
          <div className="flex items-center gap-1">
            <PortalNav items={nav} unread={unread} unreadTitle={t(locale, "nav.unreadTitle", { n: unread, plural: unread > 1 ? "s" : "" })} variant="mobile" />
          </div>
        </header>
        ...
        <aside className="hidden w-[250px] shrink-0 flex-col bg-[#0b0d13] text-white md:sticky md:top-0 md:flex md:h-screen">
          <div className="px-5 py-5">
            <BrandMark client={client} locale={locale} />
          </div>
            ...
            <div className="min-w-0 flex-1 leading-tight">
              ...
              <form action="/api/auth/logout" method="post">
                <button className="flex items-center gap-1.5 text-[11px] text-white/45 hover:text-white">
                  <LogOut size={12} /> {t(locale, "common.signOut")}
                </button>
              </form>
            </div>
...
function BrandMark({ client, locale, compact = false }) {
  return (
    <div>
      <div className="flex items-center gap-2.5">
        ...
        <div className="leading-tight">
          ...
          {!compact && (
            <div className="font-mono text-[9px] uppercase tracking-[0.14em] text-white/40">{t(locale, "nav.clientPortal")}</div>
          )}
        </div>
      </div>
      {!compact && (
