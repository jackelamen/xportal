...
export async function POST(request, { params }) {
  ...
  if (action === "toggle_visibility") {
    ...
    db.prepare("UPDATE portal_projects SET hidden_from_client = ? WHERE id = ?").run(next, id);
    logActivity({
    ...
    db.prepare(
      `UPDATE portal_projects SET
         current_phase = ?, progress_percentage = ?, target_date = ?, updated_at = datetime('now')
       WHERE id = ?`
    ).run(
    ...
    const { mx } = db
      .prepare("SELECT COALESCE(MAX(sort_order), -1) AS mx FROM project_milestones WHERE project_id = ?")
      .get(id);
    db.prepare(
    ...
    db.prepare("UPDATE project_milestones SET status = ? WHERE id = ? AND project_id = ?").run(
      String(form.get("status")), String(form.get("milestone_id")), id
    );
  } else if (action === "delete_milestone") {
    db.prepare("DELETE FROM project_milestones WHERE id = ? AND project_id = ?").run(
